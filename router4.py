"""
Router 4 Client Script

This script serves as a server for Router 4 in a network simulation. It establishes a socket connection, 
...
- processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, soc_to_router5, soc_to_router6, max_buffer_size=5120): 
...
- Run the script to start Router 4 server.
...
# Additional Helper Functions for Router4
...
def start_server():
    ...
    # Create socket for router 5
    soc_to_router5 = create_socket('127.0.0.1', 8005)
    # Create socket for router 6
    soc_to_router6 = create_socket('127.0.0.1', 8006)
...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, soc_to_router5, soc_to_router6, max_buffer_size=5120):
    """
    Handles packet processing in a separate thread.
    ...
    while True:
        ...
        if int(ttl) > 0:
            ...
            if new_ttl > 0:
                ...
                if send_to_router == '8005':
                    print(f"sending packet {new_packet} to Router 5")
                    soc_to_router5.send(new_packet.encode())
                    write_to_file(f'sent_by_router_{rnumber}.txt', new_packet, send_to_router='2')
                elif send_to_router == '8006':
                    print(f"sending packet {new_packet} to Router 6")
                    soc_to_router6.send(new_packet.encode())
