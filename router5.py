"""
Router 5 Client Script

This script serves as a client for Router 5 in a network simulation. It establishes a socket connection to a server,
...
Main Function:
- start_server(): Initializes the client, establishes connections, and starts processing threads.
...
- Run the script to start Router 5 client.
...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    while True:
        ...
        if int(ttl) > 0:
            ...
            print("DISCARD (TTL=0):", packet)
            write_to_file('discarded_by_router_5.txt', packet)
